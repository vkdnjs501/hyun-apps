/* Hyun-apps 1.3.5 — Progressive, framework-free portfolio controller. */
(() => {
  'use strict';
  ...
  function mountLogic(project, card, object, caption) {
    ...
    const describeMedia = () => {
      ...
      caption.textContent = failed ? '이미지 없이 보기 · 앱 열기 가능' : '원본 일러스트를 활용한 소개 연출';
      status.textContent = failed ? `${members[selected].name}의 그림을 불러오지 못했습니다. 앱 열기 버튼으로 원본 앱을 열 수 있습니다.` : '';
    };
  ...
  function render() {
    ...
    projects.forEach((p, index) => {
      ...
      copy.append(heading, title, el('p', 'project-subtitle', p.subtitle || ''), el('p', 'project-description', String(p.description || '').replace(/\\n/g, '\n')), actions, details);
  ...
  function measure() {
    ...
    const gutter = parseFloat(style.scrollPaddingLeft) || 0;
    cardStride = Math.max(1, (cards[0]?.offsetWidth || 0) + (parseFloat(style.columnGap) || 0));
  ...
  function setCurrent(index, announce = true) {
    ...
    cards.forEach((card, i) => {
      ...
      card.classList.remove('is-entering', 'is-first-visit');
    ...
    // Shared copy/actions enter on every visit. Only character covers enter once.
    requestAnimationFrame(() => {
      if (current !== index) return;
      cards[index].classList.add('is-entering');
      cards[index].classList.toggle('is-first-visit', firstVisit);
    });
  ...
  track.addEventListener('pointerdown', (event) => {
    if (!cards.length || event.pointerType !== 'mouse' || event.button !== 0 || event.target.closest('a, button, input, select, textarea')) return;
    stopAnimation();
